function [Par] = TPA_BehaviorEditorXY(Par)
%
        ...
        switch Cmnd,
            ...
            case 'add',
               ...
               %add the last rectangle ROI to the list of Rectangle ROIs
                ...
                clr                     = 'r';
                set(SData.strEvent{activeIndx}.XY.hShape,     'Color',     clr,'visible','on');
                set(SData.strEvent{activeIndx}.XY.hBoundBox,  'edgeColor', clr,'visible','on');
                set(SData.strEvent{activeIndx}.XY.hCornRect,  'Color', clr,'visible','on');
                set(SData.strEvent{activeIndx}.XY.hText,      'color',     clr,'visible','on');
...
%         end
    ...
    function fExportROI(~,~)
        ...
        for i=1:roiNum,
            ...
            %roiList{i}.averType = SData.strEvent{i}.AverType;
            ...
            delete(SData.strEvent{i}.XY.hCornRect);       SData.strEvent{i}.XY.hCornRect = [];
            delete(SData.strEvent{i}.XY.hText);           SData.strEvent{i}.XY.hText = [];
            
            % do not touch old data
            if roiIsBeingEdited,
                % mean if any ois invalidated
                SData.strEvent{i}.meanROI   = [];
                SData.strEvent{i}.procROI   = [];
            end;
...
%                                                    SData.imBehaive(:,:,activeZstackIndex,1:nT-2)),4));
    ...
    function fRefreshImage (~)
        ...
        for i=1:sz
            ...
            %pos                     = SData.strEvent{i}.Position;
            %xy                      = repmat(pos(1:2),5,1) + [0 0;pos(3) 0;pos(3) pos(4); 0 pos(4);0 0];
            %set(SData.strEvent{i}.XY.hShape,     'xdata',xy(:,1),'ydata',xy(:,2),'Color',color);
            %set(SData.strEvent{i}.XY.hShape,     'position',SData.strEvent{i}.Position, 'edgeColor',color);
            set(SData.strEvent{i}.XY.hShape,    'visible',showRoi,'Color',color);
            set(SData.strEvent{i}.XY.hText,     'visible',showTxt,'Color',color);
            set(SData.strEvent{i}.XY.hBoundBox,  'visible',showBox,'edgeColor',color);
            set(SData.strEvent{i}.XY.hCornRect, 'visible',showBox,'Color',color);
...
%                 set(SData.strEvent{i}.XY.hBoundBox,  'visible',showRoi,'edgeColor',color);
%                 set(SData.strEvent{i}.XY.hText,      'visible',showRoi,'Color',color);
%                 set(SData.strEvent{i}.XY.hCornRect,  'visible',showRoi,'Color',color);                
%                 
%             end
