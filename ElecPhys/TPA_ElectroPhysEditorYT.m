function [Par] = TPA_ElectroPhysEditorYT(Par)
%
% TPA_ElectroPhysEditorYT - Graphical interface to view Electrophysiology data in time. 
...
% Depend:     Global image data from Electrophysiology experiment. 
%
...
% 18.01 13.04.14 UD     adopted for Electro Phys signals
...
if nargin < 1
    ...
    Par             = TPA_ParInit();
    recordValue     = rand(30000,1);
else
    % get records
    recordValue = Par.recordData.recordValue;
end
...
[nT,nZ]             = size(recordValue);
minV                = min(recordValue);
maxV                = max(recordValue);
...
guiType                 = Par.GUI_TYPES.ELECTROPHYS_YT; % self Id
...
%             'tooltipstring',       'Setup Selected ROI Properties and Parameters ');
        ...
        imgShow                 = plot(imgAxes,recordValue);  hold on;
        imgNav                  = plot([10 10],[minV(1) maxV(1)],'color','c','LineStyle',':');       hold off;
        axis off;
...
%                 end
                set(roiLast.YT.hText,'pos',roiLast.Position(1:2)+[5,5], 'visible','on')
...
% ROI Input & Output
    ...
    function fExportROI(~,~)
        ...
        for i=1:roiNum,
            ...
            SData.strEvent{i}.tInd      = round([min(xy(:,1)) max(xy(:,1))]);  % time/frame indices
...
% Button Clicks
    ...
    function clickedHelpTool(~, ~)
        ...
        heltTxt{2} = sprintf('ElectroPhys  : Data file name   %s',      Par.DME.VideoFrontFileNames{Par.DME.Trial});
        helpdlg(heltTxt)
    end
   ...
   function fUpdateBrowseData()
       ...
       % check if the button browse is pressed
        ...
        switch guiType,
           ...
           case {Par.GUI_TYPES.TWO_PHOTON_YT,Par.GUI_TYPES.BEHAVIOR_YT,Par.GUI_TYPES.ELECTROPHYS_YT} % each new point will be absolute position
...
%                                                    SData.imBehaive(:,:,activeZstackIndex,1:nT-2)),2));
...
%         end
    ...
    function fRefreshImage (~)
        ...
        set(handStr.imgText,'string',sprintf('Z:%2d/%2d, T:%4d/%4d',activeZstackIndex,nZ,activeTimeIndex,nT))
