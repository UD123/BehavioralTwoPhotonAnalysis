...
function  [Par,FrameStart] = TPA_FrameSyncSystem2014(Par,RecordedValues)
%DTP_FrameSyncSystem2014 - System 2014 selection frame sync
% Inputs:
...
DTP_ManageText([], sprintf('ElectroPhys : yFeedback - Vertical Frame num    : %d',vertSyncNumFromYfeed),  'I' ,0);
...
% check the last one
% if (vertSyncInd(end) + searchNum) > recordNum, 
%     vertSyncInd = vertSyncInd(1:end-1); 
% end;
outOfRange              = (vertSyncInd + searchNum) > recordNum;
vertSyncInd(outOfRange) = [];
...
for k = 1:vertSyncNum,
    ...
    %validBool(k)        = all(blankBool(vertSyncInd(k)+1:vertSyncInd(k)+searchNum) == true);
end;
...
if vertSyncNum <= vertSyncNumCut,
    ...
    txt = sprintf('ElectroPhys : Blanking has shorter length than yFeedback. Cutting blanks.');
