function [ttt] = TPA_BehaviorEditorXY(ttt)
%
% TPA_BehaviorEditorXY - Graphical interface to select rectangle or/and ellipse
...
%             SData.imBehaive - (global) any 4D image array such as image=imread('demo.jpg') -
%             SData.strEvent  - (global) previous selection produced by this same application
%
% Output:     ListROI produced (see Input ROI for format)
%
%
...
% Ver	Date	 Who	Descr
%-----------------------------
% 21.02 18.08.15 UD     Support event class
% 20.13 18.08.15 UD     Fixing bugs when loading events
% 20.12 27.07.15 UD     Renaming ROIs to events - bug fixing
...
imageIn                 = squeeze(SData.imBehaive(:,:,activeZstackIndex,activeTimeIndex)); %imread(demoFile);
imageInMean             = []; % used to remember calculations of STD

% %-----------------------------------------------------
% % ROI show
% nRoi                    = length(SData.strROI);
% if nRoi < 1, % init for graphics
%     nRoi                = 1;
%     roiActivity         = zeros(nT,nRoi);
% else
%     roiActivity         = zeros(nT,nRoi);
%     for m = 1:nRoi,
%         dLen            = size(SData.strROI{m}.Data,1);
%         if dLen < 1, continue; end;
%         dffData         = SData.strROI{m}.Data(:,2);
%         roiActivity(:,m) = interp1(1:dLen,dffData,linspace(1,dLen,nT));
%         
%     end
% end
...
guiType             = Par.GUI_TYPES.BEHAVIOR_XY; % self Id
...
% Main Window
    ...
    function fCreateGUI()
        ...
        uipushtool(ht,...
            'CData',               s.ico.xp_save,...
            'clickedCallback',     {@fExportROI,1},... %@saveSession,...
            'tooltipstring',       'Save Event data to file: CTRL-s',...
            'Enable',              'on',...
            'separator',           'on');
        ...
        % roi activity grpahs
%         roiAxis               = axes();
%         set(roiAxis,  'tickdir','out', 'position',[0.01 0.03 0.98 0.05]);
%         roiPlot                = bar(roiAxis,(1:nRoi),roiActivity(1,:),0.8,'facecolor',[.1 1 .1],'LineStyle','-'); %hold off; %,'BackgroundColor','k');
%         ylim([0 2]);xlim([0 nRoi]);   axis off;
    ...
    function fManageRoiArray(Cmnd,Data)
        ...
        switch Cmnd,
            ...
            case 'add',
                ...
                maxCountId                  = 0;
                for k = 1:roiNum,
                    maxCountId = max(maxCountId,SData.strEvent{k}.CountId);
                end
...
% ROI Input & Output
    ...
    function fExportROI(~,~,onlySave)
        if nargin < 3, onlySave = 0; end;
        ...
        for i=1:roiNum,
            ...
            if onlySave < 0.5,
                ...
                [roiLast, isOK]     = SaveView(roiLast,VIEW_TYPES.XY);
            end
        ...
        if strcmp(buttonName,'Yes'),
            ...
            DTP_ManageText([], sprintf('Behavior XY : %d Events are saved',roiNum), 'I' ,0) ;
        else
            DTP_ManageText([], sprintf('Behavior XY : Events data is not changed'), 'I' ,0) ;
        end
        end
        %         else
            ...
            case GUI_STATES.ROI_INIT,
                if leftClick,
                    if yesItIs,
                        ...
                        % start drawing
                        roiType     = fGetRoiType();
                        roiLast     = TPA_EventManager(roiType,VIEW_TYPES.XY,point1(1,1:2),handStr.imgAxes);
                        %roiLast     = TPA_EventManager(roiType,point1(1,1:2),VIEW_TYPES.XY);
...
% GUI Image management
        ...
        switch imageShowState,
            case IMAGE_TYPES.RAW,
                ...
                if previousXYZTIndex(3) ~= activeZstackIndex || previousXYZTIndex(4) ~= activeTimeIndex || stateIsChanged,
                    imageIn           = squeeze(SData.imBehaive(:,:,activeZstackIndex,activeTimeIndex));
                end
            case IMAGE_TYPES.MEAN,
                %if any(activeIndexFixed(3)), return; end; % improve performance
                if previousXYZTIndex(3) ~= activeZstackIndex || stateIsChanged,
                    imageIn           = squeeze(mean(SData.imBehaive(:,:,activeZstackIndex,:),4));
                end
            case IMAGE_TYPES.MAX,
                %if any(activeIndexFixed(3)), return; end; % improve performance
                if previousXYZTIndex(3) ~= activeZstackIndex || stateIsChanged,
                    imageIn           = squeeze(max(SData.imBehaive(:,:,activeZstackIndex,:),[],4));
                end
            case IMAGE_TYPES.GRADT,
                %if any(activeIndexFixed(3)), return; end; % improve performance
                if previousXYZTIndex(3) ~= activeZstackIndex || stateIsChanged,
                    ...
                    %               imageIn           = squeeze(mean(abs(SData.imBehaive(:,:,activeZstackIndex,2:nT-1)*2 - ...
                    %                                                    SData.imBehaive(:,:,activeZstackIndex,3:nT) - ....
                    %                                                    SData.imBehaive(:,:,activeZstackIndex,1:nT-2)),4));
                    imageIn           = squeeze(mean(abs(SData.imBehaive(:,:,activeZstackIndex,1:nT-2) - ...
                        SData.imBehaive(:,:,activeZstackIndex,3:nT)),4));
            ...
            case IMAGE_TYPES.GRADXY,
                ...
                if previousXYZTIndex(3) ~= activeZstackIndex || stateIsChanged,
                    ...
                    DTP_ManageText([], sprintf('Computing Image transformation. Please wait....'), 'I' ,0);
                    imageIn          = squeeze(mean(abs(SData.imBehaive(2:nR-1,2:nC-1,activeZstackIndex,:) - ...
                        SData.imBehaive(1:nR-2,2:nC-1,activeZstackIndex,:)),4)) + ...
                        squeeze(mean(abs(SData.imBehaive(2:nR-1,2:nC-1,activeZstackIndex,:) - ...
                        SData.imBehaive(2:nR-1,1:nC-2,activeZstackIndex,:)),4));
            ...
            case IMAGE_TYPES.STD,
                %if any(activeIndexFixed(3)), return; end; % improve performance
                if previousXYZTIndex(3) ~= activeZstackIndex || stateIsChanged,
                    imageIn           = squeeze(std(abs(SData.imBehaive(:,:,activeZstackIndex,1:nT-2) - ...
                        SData.imBehaive(:,:,activeZstackIndex,3:nT)),4));
                end
    ...
    function fRefreshImage (~)
        ...
        %axes(get(handStr.roiPlot,'parent'))
        %set(handStr.roiPlot,'ydata',roiActivity(activeTimeIndex,:),'visible','on');
...
% GUI MultiWindow Synchronization
    ...
    function fSyncRecv(srcIdRx,msgObjRx)
        ...
        %fprintf('XY fSyncRecv %d : %d : %6.5f\n',srcIdRx,msgObjRx.msgId,toc);
