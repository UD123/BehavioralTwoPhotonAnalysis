classdef TPA_DataManagerPrarie
    % TPA_DataManagerPrarie - Initializes different Video directory structures used in the analysis
    ...
    % Ver	Date	 Who	Descr
    %-----------------------------
    % 29.02 20.02.19 UD     Can show multiple channels
    % 29.01 26.09.18 UD     Can load multiple channels
    ...
    properties
        ...
        VideoFrameRate              = 30;            % Two Photon Frame Rate Hz
    ...
    methods
        ...
        % ==========================================
        function obj = TPA_DataManagerPrarie()
        ...
        function obj = Clean(obj)
            ...
            obj.ChannelIndex               = 1;             % Channel index to load
        end
        ...
        % ==========================================
        function [obj,isOK] = SetChannelIndex(obj,chanIndex)
            % sets channel indexes
            
            isOK    = false;
            numChan = length(chanIndex);
            if numChan < 1 || numChan > 2
                DTP_ManageText([], sprintf('Channel number value %d is out of range. No action taken.',numChan), 'E' ,0);
                return;
            end
            
            % test existence
            if size(obj.VideoFileNames,1) < 1
                DTP_ManageText([], sprintf('Load data first.'), 'E' ,0);
                return;
            end
            
            valid = false(1,numChan);
            for k = 1:numChan
                txtToFind  = sprintf('_Ch%d_',chanIndex(k));
                valid(k)   = any(contains(obj.VideoFileNames(:,1),txtToFind));
                if ~valid(k)
                    DTP_ManageText([], sprintf('Channel index %d is not found. Correct the index. No action is taken',chanIndex(k)), 'E' ,0);
                    return;                    
                end
            end
            
            isOK             = true;
            obj.ChannelIndex = chanIndex;
        end % 
        ...
        function [obj,isOK] = GuiSetDataParameters(obj)
            ...
            prompt                  = {'Data Resolution [X [um/pix] Y [um/pix] Z [um/frame] T [frame/sec]',...
                ...
                'Channel Indexes to use :    [1,2,3... ] ',...
                'Slice Tiff File on Multiple Z Stacks [nZ - number of Z] ',...
                };
            name                ='Config Data Parameters';
            numlines            = 1;
            defaultanswer       ={num2str(obj.Resolution),num2str(obj.DecimationFactor),num2str(obj.ChannelIndex),num2str(obj.SliceNum)};
            answer              = inputdlg(prompt,name,numlines,defaultanswer,options);
            if isempty(answer), return; end
            ...
            isOK                = isOK1 && isOK2;
            chi                 = str2num(answer{3});
            [obj,isOK3]         = obj.SetChannelIndex(chi) ;
        ...
        function obj = SelectTwoPhotonData(obj,dirPath)
            ...
            DTP_ManageText([], sprintf('Prarie : %d trial data has been read successfully',dirNum), 'I' ,0)   ;
        end
        ...
        function [obj, imgData] = LoadTwoPhotonData(obj,currTrial, imageIndx)
            ...
            % inform
            DTP_ManageText([], sprintf('Prarie : Loading data from directory %s. Please Wait ...',obj.VideoDirNames{currTrial}), 'I' ,0)   ;
            ...
            if any(obj.DecimationFactor > 1)
                DTP_ManageText([], sprintf('Prarie : data from file %s is decimated. Check decimation factors ',fileDirName), 'W' ,0)   ;
                % indexing
                sz              = size(imgData);
            ...
            % check channels - set them to become slices
            chanNum             = numel(obj.ChannelIndex); nTs = 0; imgDataMerged = [];
            for k = 1:chanNum
                textToFind = sprintf('_Ch%d_',obj.ChannelIndex(k));
                validBool  = contains(obj.VideoFileNames(:,currTrial),textToFind);
                
                imgDataChan         = imgData(:,:,:,validBool);
                [nR,nC,nZ,nT]       = size(imgDataChan);
                ...
                if (obj.SliceNum > 1)
                    ...
                    imgDataChan      = reshape(imgDataChan,[nR,nC,obj.SliceNum,nTs]);

                end
                
                % merge channels in  3rd dimension
                if k > 1 && size(imgDataMerged,4) ~= size(imgDataChan,4)
                        DTP_ManageText([], sprintf('Channels contain different number of images. Please Fix it.'), 'E' ,0);
                        return;
                end
                imgDataMerged        = cat(3,imgDataMerged,imgDataChan);
            
            end
            imgData             = imgDataMerged;
            ...
            DTP_ManageText([], sprintf('Prarie : %d images are loaded from file %s successfully',obj.VideoDataSize(4),obj.VideoFileNames{currTrial}), 'I' ,0)   ;
        end
        ...
        function [obj, usrData] = LoadRoiData(obj, strROI )
            ...
            zIndList    = 1:obj.VideoDataSize(3);
            ...
            % output
            DTP_ManageText([], sprintf('Prarie : %d ROIs are assigned to Z-Stack %d successfully',roiNum,zIndTo), 'I' ,0)   ;             
        end
        % ------------------------------------------
        ...
        function obj = SaveTwoPhotonData(obj,currTrial,imgData)
            ...
            % inform
            DTP_ManageText([], sprintf('Prarie : Writing data to file %s. Please Wait ...',fileDirName), 'I' ,0)   ;
            ...
            DTP_ManageText([], sprintf('Prarie : Done'), 'I' ,0)   ;
        end
        ...
        function [obj, imgData] = ShiftTwoPhotonData(obj,imgData,currShift)
            ...
            % check
            if obj.VideoFileNum < 1
                showTxt     = sprintf('Prarie : No data found. Could be a problem with directory or experiment file tree.');
                DTP_ManageText([], showTxt, 'E' ,0) ;
                return;
            end
            [shiftNum,nPar,shiftDim]        = size(currShift);
            if shiftNum < 1 ,
                showTxt     = sprintf('Prarie : No registration data is provided. Video is untouched.');
                DTP_ManageText([], showTxt, 'W' ,0)
                return;
            end
            [nR,nC,nZ,nT]          = size(imgData);
            if shiftNum > nT,
                showTxt     = sprintf('Prarie : Registration data differes from actual %d. Trying to fix.',nT);
                DTP_ManageText([], showTxt, 'W' ,0);
                shiftNum  = nT;
            ...
            if shiftNum ~= nT,
                errordlg(sprintf('Prarie : Registration data does not match number of images. Should you specify slice number in ConfigData?'))   ;
                return;
            end
            ...
            if shiftDim > 1,
                if nZ < 2,
                    errordlg(sprintf('Prarie : Registration data expects multiple slice image data. Should you specify slice number in ConfigData?'))   ;
                    return;
                end
                % check if the number of dimensions is less than number of slices
                nZmax  = min(nZ,shiftDim);
                showTxt     = sprintf('Prarie : Registration data has %d dimensions. Image data has %d slices.',shiftDim,nZ);
            ...
            DTP_ManageText([], sprintf('Prarie : Registration data is applied'), 'I' ,0)   ;
    ...
    methods
        ...
        function obj = SelectAnalysisData(obj,dirPath)
            ...
            if isempty(dirPath),
                DTP_ManageText([], 'Prarie :  Path is not specified', 'E' ,0)
                return;
            end;
            ...
            if ~exist(dirPath,'dir'),
                ...
                isOK = mkdir(dirPath);
                if isOK,
                    showTxt     = sprintf('Prarie : Can not find directory %s. Creating one...',dirPath);
                    DTP_ManageText([], showTxt, 'W' ,0)
                else
            ...
            fileNum          = length(fileNames);
            if fileNum < 1,
                showTxt = sprintf('Prarie : Can not find data files %s in the directory %s. Check file or directory names.',obj.RoiFilePattern,dirPath);
                DTP_ManageText([], showTxt, 'W' ,0)   ;
                return
            end;
            [fileNamesC{1:fileNum,1}]   = deal(fileNames.name);
            ...
            DTP_ManageText([], sprintf('Prarie : %d analysis data files has been read successfully',fileNum), 'I' ,0)   ;
        ...
        function [obj, fileName] = GetAnalysisFileName(obj,currTrial)
            ...
            if obj.VideoDirNum < currTrial,
                DTP_ManageText([], sprintf('Prarie : Trial is out of video file range. Call 911.'), 'E' ,0) ;
                return;
            end
            
            if obj.VideoFileNum > 0,
                fileName        = obj.VideoFileNames{1,currTrial} ;
                DTP_ManageText([], sprintf('Prarie : No ROI file name is found. Trying to determine it from Video data.'), 'W' ,0) ;
            else
                DTP_ManageText([], sprintf('Prarie : No video files. Need to load video first.'), 'E' ,0) ;
                return
            end
...
%                 end
        ...
        function [obj, usrData] = LoadAnalysisData(obj,currTrial, strName)
            ...
            if ~exist(fileToLoad,'file'),
                showTxt     = sprintf('Prarie :  Analysis : Can not locate file %s. Nothing is loaded.',fileToLoad);
                DTP_ManageText([], showTxt, 'E' ,0) ;
                return;
            end
            ...
            DTP_ManageText([], sprintf('Prarie : %s data from file %s has been loaded successfully',strName,obj.RoiFileNames{currTrial}), 'I' ,0)   ;
        end
        ...
        function [obj, usrData] = SaveAnalysisData(obj,currTrial,strName,strVal)
            ...
            if currTrial > obj.VideoFileNum,
                DTP_ManageText([], sprintf('Prarie : Requested trial exceeds video files. '), 'W' ,0) ;
                %return
            end;
            if ~any(strcmp(strName,{'strROI','strShift'})),
                DTP_ManageText([], sprintf('Prarie : Input structure name must be strROI.'), 'E' ,0) ;
            ...
            if ~exist(fileToSave,'file')
                showTxt     = sprintf('Prarie : Analysis : No previous data file found. Creating a new file.');
                DTP_ManageText([], showTxt, 'W' ,0) ;
            else
                % load old data
            ...
            DTP_ManageText([], sprintf('Prarie : Analysis data has been saved to file %s',obj.RoiFileNames{currTrial}), 'I' ,0)   ;
        end
        ...
        function obj = SelectAllData(obj,dirPath)
            ...
            % check
            if ~exist(dirPath,'dir'),
                showTxt     = sprintf('Prarie : Can not find directory %s',dirPath);
                DTP_ManageText([], showTxt, 'E' ,0)
                return
            end
        ...
        function obj = RemoveRecord(obj,currTrial, removeWhat)
            ...
            %             if currTrial > obj.VideoFileNum,
            %                 DTP_ManageText([], sprintf('Prarie : Roi requested trial exceeds video files. Nothing is deleted.'), 'E' ,0) ;
            %                 return
            %             end;
        ...
        function [obj, CheckOK] = CheckData(obj, ReadDir)
            ...
            if obj.Trial < 1,
                obj.Trial       = 1;
                DTP_ManageText([], 'Prarie : No trial is selected. Please select a trial or do new data load.', 'E' ,0)   ;
            end
            ...
            if ReadDir,
                if isempty(obj.VideoDirNames),
                    DTP_ManageText([], 'Prarie : It is possible that you need to select directory above the current.', 'E' ,0)
                    %return
                else
...
%                     upDir           = upDir(1:ind(end));
            ...
            if obj.VideoFileNum < 1,
                DTP_ManageText([], 'Prarie : Video data is not found. Trying to continue - must be specified for analysis.', 'E' ,0)   ;
                CheckOK         = false;
            else
                if obj.Trial <= size(obj.VideoFileNames,2)
                    DTP_ManageText([], sprintf('Prarie : Using image data from dir %s.',obj.VideoDirNames{obj.Trial}), 'I' ,0)   ;
                end
            end
            ...
            if obj.RoiFileNum < 1,
                DTP_ManageText([], 'Prarie : User ROI analysis data is not found. Trying to continue.', 'W' ,0)   ;
            else
                if obj.Trial <= length(obj.RoiFileNames)
                    DTP_ManageText([], sprintf('Prarie : Using analysis ROI data from file %s.',obj.RoiFileNames{obj.Trial} ), 'I' ,0)   ;
                end
            end
            
            if obj.VideoDirNum ~= obj.RoiFileNum && obj.RoiFileNum > 0,
                DTP_ManageText([], 'Prarie : Video and Analysis file number missmatch.', 'W' ,0)   ;
                %imageNum         = obj.RoiFileNum;
            end
            
            % summary
            DTP_ManageText([], sprintf('Prarie : Check status %d : Found %d valid trials.',CheckOK,imageNum), 'I' ,0)   ;
    ...
    methods
        ...
        function obj = TestVideoLoad(obj)
            ...
            testVideoDir    = 'C:\LabUsers\Liora\Data\Imaging\B16\8_11_15';
            tempTrial       = 3;
            frameNum        = 31;
