function [Par] = TPA_TwoPhotonEditorYT(Par)
%
% TPA_TwoPhotonEditorYT - Graphical interface to view 3D or 4D data in time. 
...
%             SData.imTwoPhoton - (global) any 4D image array 
...
% 16.16 24.02.14 UD     Update ROI management structure
% 16.08 22.02.14 UD     Making this figure slave. ROI only display. Can not change. No context menu
...
imageIn             = squeeze(SData.imTwoPhoton(:,activeXaxisIndex,activeZstackIndex,:)); %imread(demoFile);
...
guiType             = Par.GUI_TYPES.TWO_PHOTON_YT; % self Id
...
%fLoadROI();              % load ROI structure and convert view if any
% DO NOT LOAD ANY EVENT
...
%             'tooltipstring',       'Setup Selected ROI Properties and Parameters ');
        ...
        uitoggletool(ht,...
            ...
            'tooltipstring',       'Save ROI data to file: CTRL-s',...
            'Enable',              'off',...
            'separator',           'on');
       ...
       % manage list of ROIs
        ...
        switch Cmnd,
            ...
            case 'copyFromList',
                ...
                shapeInitialDrawing         = [get(roiLast.YT.hShape,'xdata')' get(roiLast.XY.hShape,'ydata')']; 
            ...
            case 'copyToList',
                ...
                clr                     = 'y';
                set(SData.strROI{activeIndx}.YT.hShape,     'Color',     clr,'visible','on');
                set(SData.strROI{activeIndx}.YT.hBoundBox,  'edgeColor', clr,'visible','off', 'curvature', [0,0]);
                set(SData.strROI{activeIndx}.YT.hCornRect,  'Color', clr,'visible','off');
                set(SData.strROI{activeIndx}.YT.hText,      'color',     clr,'visible','on')
            ...
            case 'delete',
                ...
                %remove the rectangle at the index given
                if ~isempty(SData.strROI{activeIndx}.XY),
            ...
            case 'add',
               ...
               %add the last rectangle ROI to the list of Rectangle ROIs
                ...
                % update name
                fManageLastRoi('setName',sprintf('ROI:%2d Z:%d',activeIndx,roiLast.zInd));
                
                
                SData.strROI{activeIndx}      = roiLast;
                clr                     = 'r';
                set(SData.strROI{activeIndx}.YT.hShape,     'Color',     clr,'visible','on');
                set(SData.strROI{activeIndx}.YT.hBoundBox,  'edgeColor', clr,'visible','on');
                set(SData.strROI{activeIndx}.YT.hCornRect,  'Color', clr,'visible','on');
                set(SData.strROI{activeIndx}.YT.hText,      'color',     clr,'visible','on');
...
% GUI Image management
       ...
       set(handStr.menuImage(2:end),'checked','off')
        ...
        switch menuImageType,
         ...
         case IMAGE_TYPES.GRADXY,
               ...
               imageShowState    = IMAGE_TYPES.GRADX;
   ...
   function fComputeImageForShow()
       ...
       % NEED TO MAKE IT NICE
        ...
        switch imageShowState,
            case IMAGE_TYPES.RAW,
               %if all(activeIndexFixed(2:3)), return; end; % improve performance
                %DTP_ManageText([], sprintf('Computing Image transformation. Please wait....'), 'I' ,0);               
                imageIn           = squeeze(SData.imTwoPhoton(:,activeXaxisIndex,activeZstackIndex,:));
            case IMAGE_TYPES.MEAN,
                %if any(activeIndexFixed(3)), return; end; % improve performance
                %DTP_ManageText([], sprintf('Computing Image transformation. Please wait....'), 'I' ,0);                
              imageIn           = squeeze(mean(SData.imTwoPhoton(:,:,activeZstackIndex,:),2));
           case IMAGE_TYPES.MAX,
               %if any(activeIndexFixed(3)), return; end; % improve performance
               % DTP_ManageText([], sprintf('Computing Image transformation. Please wait....'), 'I' ,0);                
              imageIn           = squeeze(max(SData.imTwoPhoton(:,:,activeZstackIndex,:),[],2));
            case IMAGE_TYPES.GRADT,
               %if any(activeIndexFixed(3)), return; end; % improve performance
                 DTP_ManageText([], sprintf('Computing Image transformation. Please wait....'), 'I' ,0);                
              imageIn           = squeeze(mean(abs(SData.imTwoPhoton(:,:,activeZstackIndex,2:nT-1)*2 - ...
                                                   SData.imTwoPhoton(:,:,activeZstackIndex,3:nT) - ....
                                                   SData.imTwoPhoton(:,:,activeZstackIndex,1:nT-2)),2));
            ...
            case IMAGE_TYPES.GRADXY,
               ...
               imageIn          = squeeze(mean(abs(SData.imTwoPhoton(2:nR-1,2:nC-1,activeZstackIndex,:)*2 - ...
                                                       ...
                                                       SData.imTwoPhoton(2:nR-1,1:nC-2,activeZstackIndex,:)),2));
   ...
   function fRefreshImage (~)
        ...
        for i=1:sz,
            ...
            % one view protect
            if ~isfield(SData.strROI{i}.ViewYT,'hBoundBox'), 
                continue; 
            else
                if ~ishandle(SData.strROI{i}.ViewYT.hBoundBox), 
                    continue
                end
            end
            ...
            % could be that ROI was deleted
            if ~isfield(SData.strROI{i}.ViewYT,'hBoundBox'), 
                continue; 
            end
            
            % ud - make them same color as XY
            color                   = get(SData.strROI{i}.XY.hBoundBox,'edgeColor');
            set(SData.strROI{i}.YT.hShape,    'visible','off','Color',color);
            set(SData.strROI{i}.YT.hText,     'visible',showRoi,'Color',color);
            set(SData.strROI{i}.YT.hBoundBox, 'visible',showRoi);
            set(SData.strROI{i}.YT.hCornRect, 'visible','off');  % all the active rectangles - hide

            
            
%             set(SData.strROI{i}.YT.hShape,    'visible',showRoi,'Color',color);
%             set(SData.strROI{i}.YT.hText,     'visible',showRoi,'Color',color);
%             set(SData.strROI{i}.YT.hBoundBox,  'visible','off');
%             set(SData.strROI{i}.YT.hCornRect, 'visible','off');  % all the active rectangles - hide
% 
%             
%             if strcmp(color,'r')
%                 %draw small squares in the corner and middle of length
%                 set(SData.strROI{i}.YT.hBoundBox,  'visible',showRoi,'edgeColor',color);
%                 set(SData.strROI{i}.YT.hText,      'visible',showRoi,'Color',color);
%                 set(SData.strROI{i}.YT.hCornRect,  'visible',showRoi,'Color',color);                
%                 
%             end
...
%         
        ...
        if updateRefreshImage || updateShowImage,
            fRefreshImage();
        end
    end
