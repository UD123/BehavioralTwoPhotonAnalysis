...
% strEvent.procROI     = [Nx1];         % motion info vector


%-----------------------------
% Ver	Date	 Who	Descr
%-----------------------------
% 20.02 07.03.15 UD     Events from motion
...
analysisDir         = 'C:\Uri\DataJ\Janelia\Analysis\M2\4_4_14\';
...
%%%
fileNames        = dir(fullfile(analysisDir,'EDA_*.mat'));
fileNum          = length(fileNames);
if fileNum < 1,
...
motionData       = allTrialEvents{trialIndShow}{1}.procROI; % actual data
[framNum,~]      = size(motionData);
eventDataArray   = zeros(framNum,eventsPerTrialNum);
eventNames       = cell(1,eventsPerTrialNum);

% collect
for m = 1:eventsPerTrialNum,
    motionData   = allTrialEvents{trialIndShow}{m}.procROI;
    if isempty(motionData),
        error('Behavior data is not complete. Can not find Event Motion results.')
    end
    
    % assign to vector
    eventDataArray(:,m) = motionData;
    eventNames{m} = allTrialEvents{trialIndShow}{m}.Name; 
end
timeEvent       = (1:framNum)/frameRateRatio;
...
figure(2),
plot(timeEvent,eventDataArray),legend(eventNames)
