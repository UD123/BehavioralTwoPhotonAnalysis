...
classdef TPA_CustomAlgorithmLabeler < vision.labeler.AutomationAlgorithm & vision.labeler.mixin.Temporal
    ...
    methods
        ...
        function initialize(algObj, I, labelsToAutomate)
            ...
            %--------------------------------------------------------------
             % init
            algObj.SaveDir             = '..\DataForCDNN';
            algObj.SaveFile            = 'DataCNN_D12_N21.mat';
            %addpath(algObj.SaveDir);
            
            load(fullfile(algObj.SaveDir,algObj.SaveFile),'net');
            algObj.Net                 = net;
        end
        ...
        function autoLabels = run(algObj, I)
            ...
            [bbox, score, label]    = detect(algObj.Net, uint8(I(:,:,1)), 'MiniBatchSize', 64, 'SelectStrongest', true);
                       
            %autoLabels = [];hbox
            %autoLabels = [bbox(label=='h',:);bbox(label=='m',:)];
            
            autoLabels = struct('Type','Rectangle','Name','h','Position',bbox(label=='h',:));
        end
